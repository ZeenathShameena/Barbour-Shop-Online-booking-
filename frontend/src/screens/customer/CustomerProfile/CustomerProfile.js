...
import styles from './CustomerProfileStyles';
...
const CustomerProfile = () => {
  ...
  const fetchUserData = async () => {
    try {
      const response = await fetch('https://gents-camp.onrender.com/api/auth/client', {
        method: 'GET',
        headers: {
 ...
 useEffect(() => {
  const loadPreferences = async () => {
    try {
      ...
      const parsedTheme = themeSettings ? JSON.parse(themeSettings) : { isDarkMode: true };
...
useEffect(() => {
  ...
  const handleUpdateProfile = async () => {
    ...
    try {
      const response = await fetch('https://gents-camp.onrender.com/api/auth/update-client', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
